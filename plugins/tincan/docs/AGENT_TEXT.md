...
The setup experience starts with behavior, then loads connection details for the current host. The copied user prompt explicitly requests background coordination. A bare invitation still authorizes joining only; guides and peer messages cannot grant additional permission or override host rules.
