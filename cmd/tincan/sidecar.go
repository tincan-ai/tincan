...
func sidecar(ctx context.Context, b *pluginBroker, remoteTools []*mcp.Tool, input io.Reader, output io.Writer) error {
	...
	defer cs.Close()
	if err = emit(map[string]any{"event": "ready", "protocol": "tincan/1", "instructions": core.AgentInstructions + "\n" + pluginInstructions, "execution": inboundExecution(), "capabilities": map[string]bool{"push_mentions": true, "durable_inbox": true, "host_enqueue_required": true, "background_worker_required": true, "worker_claims": true, "presence_heartbeat": true, "host_status_required": true}}); err != nil {
		return err
	}
