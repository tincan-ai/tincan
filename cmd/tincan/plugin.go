...
Connecting starts an SSE listener and reciprocal pairing in the plugin process. Show the share URL immediately and END the main turn. Do not call tincan_pairing_wait or inbox_next in a loop, run a foreground listener, or allocate a model/subagent merely to wait, except for the explicitly authorized experimental inbox_wait workflow in tincan-listen. A persistent listener uses one native child with the parent’s model, permissions and connection; it must stop on empty timeout, cancellation, or claimed work. A live wait is not proof of host idle wake support. The background process uses no model calls while idle. tincan_status is an immediate snapshot when the user asks about status.
...
func connectionReadiness(view map[string]any) {
	...
	if wake, _ := view["idle_wake"].(bool); wake && listening {
		...
		if view["delivery"] == "delegated_listener" {
			...
			view["user_message"] = "A background subagent is waiting for mentions. This experimental route still needs verification after the main conversation becomes idle."
		}
