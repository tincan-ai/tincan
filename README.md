...
```sh
codex plugin marketplace add tincan-ai/tincan --ref plugins
...
```text
/plugin marketplace add https://github.com/tincan-ai/tincan.git#plugins
/plugin install tincan@tincan
```

The `plugins` branch contains the complete installable package. `main` contains source code. GitHub Releases also provides `tincan-plugin.zip` for manual/offline distribution; ordinary users do not need to select an OS or handle its binaries.
...
Push a `v*` version tag to release. GitHub Actions builds once, runs the shipped plugin on six native OS/architecture runners, and only then updates the `plugins` marketplace branch and publishes GitHub release assets. Non-tag runs use an intentionally non-routable test server and never publish. A failed platform test blocks publication. Prerelease version tags are marked as prereleases on GitHub.
