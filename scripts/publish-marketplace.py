#!/usr/bin/env python3
"""Publish a verified universal bundle on the dedicated plugins branch."""
import argparse
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='tincan-publish-') as tmp:
        ...
        if git('ls-remote', '--heads', 'origin', 'plugins', cwd=tree):
            git('fetch', 'origin', 'plugins', cwd=tree)
            git('checkout', '-B', 'plugins', 'FETCH_HEAD', cwd=tree)
        else:
            git('checkout', '--orphan', 'plugins', cwd=tree)
        ...
        git('push', 'origin', 'HEAD:refs/heads/plugins', cwd=tree)
        print('Published the verified universal plugin to the plugins branch.')
